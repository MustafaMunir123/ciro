...
export async function generateMissionReport(incidents: Incident[], logs: string[]): Promise<MissionReport> {
    ...
    if (!process.env.GEMINI_API_KEY) {
        ...
        return {
            report_id: `OPS-${new Date().toISOString().split('T')[0]}-OFFLINE-${String(Date.now()).slice(-4)}`,
    ...
    const prompt = `You are a professional Government Emergency Response Analyst for this emergency response project.
...
1. report_id: Format "OPS-[DATE]-[TYPE]-[NUMBER]"
...
Return valid JSON only.`;
    ...
    try {
        ...
        return {
            report_id: data.report_id || `OPS-${new Date().toISOString().split('T')[0]}-OPS-${String(Date.now()).slice(-4)}`,
        ...
        return {
            report_id: `OPS-${new Date().toISOString().split('T')[0]}-ERR-${String(Date.now()).slice(-4)}`,
            classification: "OFFICIAL",
            incident_period: {
                start: startTime,
                end: endTime,
