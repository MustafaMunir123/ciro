...
// Video flow is intentionally disabled for this deployment.
...
function getFallbackRouting(incident: Incident): RoutingDecision {
    ...
    if (isVideo) {
        target = "TRIAGE";
        reasoning = "Fallback: Video flow disabled, input rerouted to Triage Agent";
...
export async function coordinateIncident(incident: Incident): Promise<Incident> {
    if (!VIDEO_FLOW_ENABLED && incident.type === "VIDEO") {
        incident = {
            ...incident,
            type: "TEXT",
    ...
    try {
        ...
        if (incident.type === "COMMAND") {
            ...
            if (!VIDEO_FLOW_ENABLED && routingDecision.target_agent === "SURVEILLANCE") {
                routingDecision = {
                    ...routingDecision,
                    target_agent: "TRIAGE",
                    reasoning: `${routingDecision.reasoning} | Video flow disabled, rerouted to TRIAGE.`,
                };
            }
            ...
            if (routingDecision.target_agent === "TRIAGE") {
                ...
                routingTrace += "Video flow disabled. Routing to Triage Agent... ";
        ...
        if (incident.type === "COMMAND" && processedIncident.command_intent) {
            ...
            if (isVideoRelated) {
                console.log(`[COORDINATOR] Command \"${intent}\" detected as video-related, but video flow is disabled. Re-routing to Triage Agent...`);
                routingTrace += ` [COMMAND REROUTE] \"${intent}\" → Triage Agent (video flow disabled)... `;

                const triageResult = await triageIncident({
                    ...processedIncident,
                    type: "TEXT",
                    mission_context: `[COMMAND VIDEO REQUEST - TRIAGE FALLBACK]: ${intent}\n${processedIncident.mission_context || ""}`
                });
