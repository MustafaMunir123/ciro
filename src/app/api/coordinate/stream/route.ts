...
const VIDEO_FLOW_ENABLED = false;
...
export async function GET() {
    ...
    try {
        ...
        return NextResponse.json({ status: "READY", message: "AI Core Warmup Complete" });
    } catch (e: any) {
...
export async function POST(req: NextRequest) {
    const incomingIncident: Incident = await req.json();
    const incident: Incident = (!VIDEO_FLOW_ENABLED && incomingIncident.type === "VIDEO")
        ? {
            ...incomingIncident,
            type: "TEXT",
            mission_context: `[VIDEO FLOW DISABLED - AUTO TRIAGE MODE]\n${incomingIncident.mission_context || ""}`,
        }
        : incomingIncident;
    ...
    const systemInstruction = `You are the Coordinator. You are the "Traffic Cop" for an emergency response system. 
    ...
    const stream = new ReadableStream({
        async start(controller) {
            ...
            try {
                ...
                const connectEvent = { type: "thought", content: " [SYSTEM] Connecting to satellite network..." };
                ...
                if (!routingResult) {
                    ...
                    if (inputLower.endsWith(".mov") || inputLower.endsWith(".mp4") || inputLower.endsWith(".avi")) {
                        fallbackTarget = VIDEO_FLOW_ENABLED ? "SURVEILLANCE" : "TRIAGE";
                ...
                const targetAgent = (!VIDEO_FLOW_ENABLED && routingResult.target_agent === "SURVEILLANCE")
                    ? "TRIAGE"
                    : routingResult.target_agent;

                if (!VIDEO_FLOW_ENABLED && routingResult.target_agent === "SURVEILLANCE") {
                    const rerouteEvent = { type: "thought", content: "[SYSTEM] Video flow disabled. Re-routing SURVEILLANCE request to TRIAGE." };
                    safeEnqueue(encoder.encode(JSON.stringify(rerouteEvent) + "\n"));
                    routingResult.reasoning = `${routingResult.reasoning || ""} | Video flow disabled, rerouted to TRIAGE.`.trim();
                }
                ...
                if (targetAgent === "TRIAGE") {
                    ...
                    // Safety fallback if explicitly invoked.
                    safeEnqueue(encoder.encode(JSON.stringify({ type: "agent_info", agent: "Triage Agent", model: MODELS.TRIAGE }) + "\n"));
                    const { triageIncident } = await import("@/agents/triage");
                    agentResult = await triageIncident({
                        ...incident,
                        type: "TEXT",
                        mission_context: `[VIDEO FLOW DISABLED - FALLBACK TO TRIAGE]\n${incident.mission_context || ""}`,
                    }, streamThought);
