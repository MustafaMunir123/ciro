...
const karachiEntry = (pakistanAreaTopics as Array<{ city: string; areas: Array<{ lat: number; lng: number }> }>)
    .find((entry) => entry.city.toLowerCase() === "karachi");
const firstKarachiArea = karachiEntry?.areas?.[0];
const DEFAULT_CENTER = firstKarachiArea
    ? { lat: firstKarachiArea.lat, lng: firstKarachiArea.lng }
    : { lat: 24.8607, lng: 67.0011 }; // Karachi fallback
const isPakistanBounds = (lat?: number, lng?: number) =>
    typeof lat === "number" &&
    typeof lng === "number" &&
    lat >= 23 &&
    lat <= 38 &&
...
function MapController({ incidents }: { incidents: Incident[] }) {
    ...
    useEffect(() => {
        ...
        if (incident?.location?.lat && incident?.location?.lng && isPakistanBounds(incident.location.lat, incident.location.lng)) {
...
export function TacticalMap({ className }: { className?: string }) {
    ...
    if (!isMounted) {
        return (
            <div className={`flex flex-col items-center justify-center bg-zinc-950 border border-zinc-800 rounded-lg ${className}`}>
                ...
                <span className="text-xs font-mono text-blue-300 tracking-[0.2em] animate-pulse">
    ...
    const firstWithLocation = incidents.find(i =>
        i.location &&
        i.location.lat !== null &&
        i.location.lng !== null &&
        isPakistanBounds(i.location.lat, i.location.lng)
    );
    ...
    const visibleIncidents = incidents.filter(incident =>
        ...
        isPakistanBounds(incident.location.lat, incident.location.lng) &&
    ...
    return (
        <div className={`relative h-full w-full rounded-xl overflow-hidden border border-zinc-800 bg-zinc-950 shadow-2xl ${className}`}>
            ...
            <div className="absolute top-4 left-4 right-4 z-[400] flex justify-between items-start pointer-events-none">
                <div className="glass-panel px-3 py-1.5 rounded-md flex items-center gap-3">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-mono font-bold text-blue-300 tracking-wider">LIVE FEED</span>
                    </div>
                </div>
            ...
            {/* 4. Dual Horizontal Scan Lines */}
            <div className="absolute inset-0 pointer-events-none z-[380] overflow-hidden">
                <div
                    className="absolute left-0 right-0 h-[4px] bg-gradient-to-r from-transparent via-blue-300 to-transparent shadow-[0_0_22px_rgba(59,130,246,0.9)] animate-scan-down-full opacity-95"
                />
                <div
                    className="absolute left-0 right-0 h-[4px] bg-gradient-to-r from-transparent via-cyan-200 to-transparent shadow-[0_0_22px_rgba(34,211,238,0.85)] animate-scan-up-full opacity-90"
